...
public class GuardianRegistrationService {
    ...
    public GuardianRegistrationService(GuardianRepository guardianRepository, PasswordEncoder passwordEncoder) {
        this.guardianRepository = guardianRepository;
        this.passwordEncoder = passwordEncoder;
    }
    
    public Guardian registerGuardian(String username, String email, String password, String name, LocalDate birthDate) {
        validateRegistrationData(username, email, password, name, birthDate);
        
        if (guardianRepository.existsByUsername(username)) {
            throw new GuardianAlreadyExistsException("Guardian with username '" + username + "' already exists");
        }
        
        if (guardianRepository.existsByEmail(email)) {
            throw new GuardianAlreadyExistsException("Guardian with email '" + email + "' already exists");
        }
    ...
    private void validateRegistrationData(String username, String email, String password, String name, LocalDate birthDate) {
        ...
        if (!username.matches("^[a-zA-Z0-9_]+$")) {
            throw new InvalidGuardianDataException("Username can only contain letters, numbers, and underscores");
        }
