...
public interface StepRecordJpaRepository extends JpaRepository<StepRecordEntity, Long> {
    ...
    @Query(
            "SELECT s FROM StepRecordEntity s WHERE s.guardianId = :guardianId AND DATE(s.timestamp) = :date")
    List<StepRecordEntity> findByGuardianIdAndDate(
            @Param("guardianId") Long guardianId, @Param("date") LocalDate date);

    @Query(
            "SELECT s FROM StepRecordEntity s WHERE s.guardianId = :guardianId AND DATE(s.timestamp) BETWEEN :fromDate AND :toDate")
