...
public class StepController {
    ...
    @Autowired
    public StepController(StepTrackingApplicationService stepTrackingService) {
    ...
    @Operation(
        ...
        description = "Submit step count for a guardian. Steps are converted to energy (1 energy per 10 steps) and aggregated daily."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "Steps submitted successfully",
    ...
    public ResponseEntity<StepSubmissionResponse> submitSteps(
            ...
            @Parameter(description = "Step submission details", required = true) @Valid @RequestBody StepSubmissionRequest request) {
        logger.info("Received step submission request for guardian {}: {} steps", guardianId, request.stepCount());
    ...
    @Operation(
        ...
        description = "Retrieve the current step count and available energy for today for a specific guardian."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Current step count retrieved successfully",
            content = @Content(schema = @Schema(implementation = CurrentStepCountResponse.class))),
        @ApiResponse(responseCode = "404", description = "Guardian not found")
    })
    ...
    @Operation(
        ...
        description = "Retrieve step history for a guardian within a specified date range. Maximum range is 30 days."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Step history retrieved successfully",
            content = @Content(schema = @Schema(implementation = StepHistoryResponse.class))),
        @ApiResponse(responseCode = "400", description = "Invalid date range"),
        @ApiResponse(responseCode = "404", description = "Guardian not found")
    })
    public ResponseEntity<StepHistoryResponse> getStepHistory(
            @Parameter(description = "Guardian ID", required = true) @PathVariable Long guardianId,
            @Parameter(description = "Start date (YYYY-MM-DD)", required = true) @RequestParam("from") LocalDate fromDate,
            @Parameter(description = "End date (YYYY-MM-DD)", required = true) @RequestParam("to") LocalDate toDate) {
        if (fromDate.isAfter(toDate)) {
            throw new IllegalArgumentException("Invalid date range: from date cannot be after to date");
        }
        
        StepHistoryResponse response = stepTrackingService.getStepHistory(guardianId, fromDate, toDate);
        return ResponseEntity.ok(response);
    }
}