...
public interface DailyStepAggregateJpaRepository extends JpaRepository<DailyStepAggregateEntity, Long> {
    
    Optional<DailyStepAggregateEntity> findByGuardianIdAndDate(Long guardianId, LocalDate date);
    
    @Query("SELECT d FROM DailyStepAggregateEntity d WHERE d.guardianId = :guardianId AND d.date BETWEEN :fromDate AND :toDate ORDER BY d.date")
