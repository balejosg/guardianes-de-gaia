...
class GuardianRegistrationServiceTest {
  ...
  void shouldThrowExceptionWhenUsernameAlreadyExists() {
    ...
    // When & Then
    GuardianAlreadyExistsException exception =
        assertThrows(
            GuardianAlreadyExistsException.class,
            () -> registrationService.registerGuardian(username, email, password, name, birthDate));
  ...
  void shouldThrowExceptionWhenEmailAlreadyExists() {
    ...
    assertEquals(
        "Guardian with email 'existing@example.com' already exists", exception.getMessage());
  ...
  void shouldThrowExceptionForInvalidUsername() {
    ...
    String[] invalidUsernames = {
      null, "", " ", "ab", "a".repeat(21), "user@name", "user name", "user-name"
    };

    for (String invalidUsername : invalidUsernames) {
      // When & Then
      assertThrows(
          InvalidGuardianDataException.class,
          () ->
              registrationService.registerGuardian(
                  invalidUsername,
                  "test@example.com",
                  "password123",
                  "Test User",
  ...
  void shouldThrowExceptionForInvalidEmail() {
    ...
    for (String invalidEmail : invalidEmails) {
      ...
      assertThrows(
          ...
          () ->
              registrationService.registerGuardian(
                  "testuser", invalidEmail, "password123", "Test User", LocalDate.of(2010, 1, 15)));
    }
  }
  ...
  void shouldThrowExceptionForInvalidBirthDate() {
    ...
    for (LocalDate invalidBirthDate : invalidBirthDates) {
      ...
      assertThrows(
          ...
          () ->
              registrationService.registerGuardian(
                  "testuser", "test@example.com", "password123", "Test User", invalidBirthDate));
    }
  }
    ...
    when(guardianRepository.save(any(Guardian.class)))
        .thenReturn(
            new Guardian(
                ...
                com.guardianes.guardian.domain.model.GuardianLevel.INITIATE,
                0,
                0,
                0,
  ...
  void shouldAcceptValidEmails() {
    ...
    for (String validEmail : validEmails) {
      ...
      assertDoesNotThrow(
          () ->
              registrationService.registerGuardian(
                  "testuser", validEmail, "password123", "Test User", LocalDate.of(2010, 1, 15)));
    }
  }
}
