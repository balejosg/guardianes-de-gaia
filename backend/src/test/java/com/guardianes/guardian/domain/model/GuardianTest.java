...
class GuardianTest {
  ...
  void shouldIdentifyChildCorrectly() {
    ...
    Guardian childGuardian =
        new Guardian("child", "child@example.com", "hash", "Child", childBirthDate);

    // Adult Guardian (age 25)
    LocalDate adultBirthDate = LocalDate.now().minusYears(25);
    Guardian adultGuardian =
        new Guardian("adult", "adult@example.com", "hash", "Adult", adultBirthDate);
  ...
  private Guardian createTestGuardian() {
    return new Guardian(
        "testuser", "test@example.com", "hashedPassword", "Test User", LocalDate.of(2010, 1, 15));
  }
}
