 ...
 *
 * <p>Provides utilities for validating health endpoints and ensuring production readiness of the
 * application stack.
 */
public class HealthCheckUtils {

  /** Validates that the health endpoint returns a healthy status */
  public static void validateHealthEndpoint(
      HttpClient client, GenericContainer<?> backend, String basicAuth) throws Exception {
    HttpResponse<String> response =
        DockerTestUtils.makeAuthenticatedRequest(client, backend, "/actuator/health", basicAuth);
  ...
  public static void validateActuatorEndpoints(
      HttpClient client, GenericContainer<?> backend, String basicAuth) throws Exception {
    String[] endpoints = {
    ...
    for (String endpoint : endpoints) {
      HttpResponse<String> response =
          DockerTestUtils.makeAuthenticatedRequest(client, backend, endpoint, basicAuth);
      assertEquals(200, response.statusCode(), "Endpoint " + endpoint + " should be accessible");
    }
  }

  /** Validates that API endpoints are functional */
  public static void validateApiEndpoints(
      HttpClient client, GenericContainer<?> backend, String basicAuth) throws Exception {
    // Test current steps endpoint
    HttpResponse<String> stepsResponse =
        DockerTestUtils.makeAuthenticatedRequest(
    ...
    assertTrue(
        DockerTestUtils.validateJsonResponse(
            stepsResponse.body(), "guardianId", "currentSteps", "availableEnergy", "date"),
        "Steps response should contain expected fields");

    // Test energy balance endpoint
    HttpResponse<String> energyResponse =
        DockerTestUtils.makeAuthenticatedRequest(
    ...
    assertTrue(
        DockerTestUtils.validateJsonResponse(
            energyResponse.body(), "guardianId", "currentBalance", "transactionSummary"),
        "Energy response should contain expected fields");
  }

  /** Validates that authentication is properly enforced */
  public static void validateAuthentication(HttpClient client, GenericContainer<?> backend)
      throws Exception {
    // Test that unauthenticated requests are rejected
    HttpResponse<String> unauthResponse =
        DockerTestUtils.makeUnauthenticatedRequest(client, backend, "/actuator/health");
    assertEquals(401, unauthResponse.statusCode(), "Unauthenticated requests should be rejected");

    // Test that invalid credentials are rejected
    String invalidAuth = DockerTestUtils.createBasicAuth("invalid", "invalid");
    HttpResponse<String> invalidResponse =
        DockerTestUtils.makeAuthenticatedRequest(client, backend, "/actuator/health", invalidAuth);
    assertEquals(401, invalidResponse.statusCode(), "Invalid credentials should be rejected");
  }

  /** Validates that the backend connects to all required external services */
  public static void validateExternalServiceConnectivity(
      HttpClient client, GenericContainer<?> backend, String basicAuth) throws Exception {
    HttpResponse<String> response =
        DockerTestUtils.makeAuthenticatedRequest(client, backend, "/actuator/health", basicAuth);
    ...
    assertFalse(logs.contains("Connection refused"), "Should not have connection refused errors");
    assertFalse(logs.contains("Unable to connect"), "Should not have connectivity errors");
    assertFalse(logs.contains("Connection timeout"), "Should not have connection timeouts");
  }
  ...
  public static void validateNoStartupErrors(GenericContainer<?> backend) {
    ...
    assertFalse(
        logs.contains("Cannot invoke \"jdk.internal.platform.CgroupInfo"),
        "Should not have cgroup issues");
    assertFalse(logs.contains("ProcessorMetrics"), "Should not have ProcessorMetrics issues");
    assertFalse(logs.contains("TomcatMetricsBinder"), "Should not have TomcatMetricsBinder issues");
  }
  ...
  public static void validatePerformance(
    ...
    assertTrue(
        healthTime < 5000,
        "Health endpoint should respond within 5 seconds, took: " + healthTime + "ms");
    ...
    assertTrue(
        apiTime < 10000, "API endpoint should respond within 10 seconds, took: " + apiTime + "ms");
  }

  /** Waits for the backend to be fully started and healthy */
  public static boolean waitForBackendHealth(
      HttpClient client, GenericContainer<?> backend, String basicAuth, int timeoutSeconds) {
    long startTime = System.currentTimeMillis();
  ...
  public static void validateMonitoringIntegration(
      HttpClient client, GenericContainer<?> backend, String basicAuth) throws Exception {
    // Prometheus metrics should be available
    HttpResponse<String> prometheusResponse =
        DockerTestUtils.makeAuthenticatedRequest(
    ...
    HttpResponse<String> metricsResponse =
        DockerTestUtils.makeAuthenticatedRequest(client, backend, "/actuator/metrics", basicAuth);
