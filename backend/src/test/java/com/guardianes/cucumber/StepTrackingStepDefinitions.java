...
public class StepTrackingStepDefinitions {
  ...
  public void i_have_taken_steps_today(Integer steps) {
    List<StepRecord> todaySteps =
        List.of(new StepRecord(guardianId, steps, currentDate.atTime(8, 0)));
    when(stepRepository.findByGuardianIdAndDate(guardianId, currentDate)).thenReturn(todaySteps);
  ...
  public void i_try_to_record_steps(Integer steps) {
    LocalDateTime timestamp =
        LocalDateTime.of(
            currentDate.getYear(), currentDate.getMonth(), currentDate.getDayOfMonth(), 14, 0);
  ...
  public void i_complete_my_daily_walking_with_steps(Integer totalSteps) {
    ...
    when(energyRepository.saveTransaction(any(EnergyTransaction.class))).thenReturn(transaction);
  ...
  public void i_should_see_energy_points_equivalent(Integer expectedEnergy) {
    int calculatedEnergy =
        energyCalculationService.calculateEnergyFromSteps(currentDayAggregate.getTotalSteps());
  ...
  public void i_record_steps_multiple_times_throughout_the_day(DataTable dataTable) {
    ...
    when(stepRepository.findByGuardianIdAndDate(guardianId, currentDate)).thenReturn(stepRecords);
  ...
  public void i_have_a_normal_step_pattern_averaging_steps_daily(Integer averageSteps) {
    ...
    when(stepRepository.findByGuardianIdAndDateRange(eq(guardianId), any(), any()))
        .thenReturn(
            List.of(
                new StepRecord(
                    guardianId, averageSteps - 200, currentDate.minusDays(2).atTime(10, 0)),
                new StepRecord(
                    guardianId, averageSteps + 100, currentDate.minusDays(1).atTime(10, 0))));
  }
  ...
  public void i_try_to_submit_steps_again() {
    ...
    lastValidationResult =
        stepValidationService.validateStepCount(guardianId, 1000, LocalDateTime.now());
  ...
  public void i_try_to_submit_steps_with_a_future_timestamp() {
    ...
    lastValidationResult =
        stepValidationService.validateStepCount(guardianId, 1000, futureTimestamp);
