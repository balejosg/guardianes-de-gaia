...
class EnergyControllerTest {
  ...
  void setUp() {
    ...
    spendingResponse =
        new EnergySpendingResponse(
            1L, 400, 100, EnergySpendingSource.BATTLE, "Energy spent successfully");
  }
  ...
  void shouldGetEnergyBalanceSuccessfully() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/v1/guardians/{guardianId}/energy/balance", guardianId))
  ...
  void shouldReturn400ForInvalidSpendingAmount() throws Exception {
    // Given
    Long guardianId = 1L;
    EnergySpendingRequest invalidRequest =
        new EnergySpendingRequest(-50, EnergySpendingSource.BATTLE);
