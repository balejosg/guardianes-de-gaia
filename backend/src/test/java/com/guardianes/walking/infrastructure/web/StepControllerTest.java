...
class StepControllerTest {
    ...
    void shouldSubmitStepsSuccessfully() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/v1/guardians/{guardianId}/steps", guardianId)
    ...
    void shouldReturn400ForInvalidStepCount() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/guardians/{guardianId}/steps", guardianId)
                ...
                .andExpect(jsonPath("$.fieldErrors[*].message").value(hasItem("Step count must be positive")));
    ...
    void shouldGetCurrentStepCount() throws Exception {
        // Given
        Long guardianId = 1L;
        CurrentStepCountResponse response = new CurrentStepCountResponse(guardianId, 4200, 420, LocalDate.now());
    ...
    void shouldGetStepHistory() throws Exception {
        ...
        StepHistoryResponse response = new StepHistoryResponse(
            guardianId,
            Arrays.asList(
                new DailyStepAggregate(guardianId, LocalDate.of(2025, 7, 1), 2000),
                new DailyStepAggregate(guardianId, LocalDate.of(2025, 7, 2), 3000),
                new DailyStepAggregate(guardianId, LocalDate.of(2025, 7, 3), 4000)
            )
        );
        
        when(stepTrackingService.getStepHistory(guardianId, fromDate, toDate))
            .thenReturn(response);
        ...
        mockMvc.perform(get("/api/v1/guardians/{guardianId}/steps/history", guardianId)
                ...
                .andExpect(jsonPath("$.dailySteps[0].totalSteps").value(2000))
                .andExpect(jsonPath("$.dailySteps[1].totalSteps").value(3000))
                .andExpect(jsonPath("$.dailySteps[2].totalSteps").value(4000));
    ...
    void shouldReturn400ForInvalidDateRange() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/guardians/{guardianId}/steps/history", guardianId)
                ...
                .andExpect(jsonPath("$.message").value("Invalid date range: from date cannot be after to date"));
