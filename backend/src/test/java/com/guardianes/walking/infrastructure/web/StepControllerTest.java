...
class StepControllerTest {
  ...
  void shouldReturn400ForInvalidStepCount() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.fieldErrors[*].message").value(hasItem("Step count must be positive")));
  ...
  void shouldGetCurrentStepCount() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/v1/guardians/{guardianId}/steps/current", guardianId))
  ...
  void shouldReturn400ForInvalidDateRange() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message").value("Invalid date range: from date cannot be after to date"));
