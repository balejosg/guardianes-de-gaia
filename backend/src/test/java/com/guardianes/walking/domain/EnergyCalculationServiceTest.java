...
class EnergyCalculationServiceTest {
  ...
  void setUp() {
    ...
    energyCalculationService =
        new EnergyCalculationService(energyRepository, stepAggregationService);
  }
  ...
  void shouldConvertDailyStepsToEnergyAndCreateTransaction() {
    ...
    when(stepAggregationService.aggregateDailySteps(guardianId, date)).thenReturn(stepAggregate);

    EnergyTransaction expectedTransaction =
  ...
  void shouldGetCurrentEnergyBalanceForGuardian() {
    ...
    List<EnergyTransaction> transactions =
        Arrays.asList(
            new EnergyTransaction(
                guardianId, EnergyTransactionType.EARNED, 100, "DAILY_STEPS", LocalDateTime.now()),
            new EnergyTransaction(
                guardianId, EnergyTransactionType.EARNED, 200, "DAILY_STEPS", LocalDateTime.now()),
            new EnergyTransaction(
                guardianId, EnergyTransactionType.SPENT, 50, "BATTLE", LocalDateTime.now()));

    when(energyRepository.findTransactionsByGuardianId(guardianId)).thenReturn(transactions);
    ...
    List<EnergyTransaction> transactions =
        Arrays.asList(
            new EnergyTransaction(
                guardianId, EnergyTransactionType.EARNED, 100, "DAILY_STEPS", LocalDateTime.now()),
            new EnergyTransaction(
                guardianId, EnergyTransactionType.SPENT, 150, "BATTLE", LocalDateTime.now()));
  ...
  void shouldSpendEnergyAndCreateTransaction() {
    ...
    EnergyTransaction expectedTransaction =
        new EnergyTransaction(
            guardianId, EnergyTransactionType.SPENT, energyToSpend, source, LocalDateTime.now());
    when(energyRepository.saveTransaction(any(EnergyTransaction.class)))
        .thenReturn(expectedTransaction);
  ...
  void shouldGetEnergyTransactionHistoryForGuardian() {
    ...
    List<EnergyTransaction> expectedTransactions =
        Arrays.asList(
            new EnergyTransaction(
                guardianId,
                EnergyTransactionType.EARNED,
                100,
                "DAILY_STEPS",
                fromDate.atTime(10, 0)),
            new EnergyTransaction(
                guardianId,
                EnergyTransactionType.SPENT,
                50,
                "BATTLE",
                fromDate.plusDays(1).atTime(14, 0)));

    when(energyRepository.findTransactionsByGuardianIdAndDateRange(guardianId, fromDate, toDate))
        .thenReturn(expectedTransactions);

    // When
    List<EnergyTransaction> result =
        energyCalculationService.getEnergyHistory(guardianId, fromDate, toDate);
    ...
    verify(energyRepository, times(1))
        .findTransactionsByGuardianIdAndDateRange(guardianId, fromDate, toDate);
  }
}
