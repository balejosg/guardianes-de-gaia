 ...
 *
 * <p>This test class validates that our Spring Boot application can start successfully in a Docker
 * container environment, preventing issues like: - Java 17 cgroup access problems - Spring Boot
 * Actuator metrics failures (ProcessorMetrics, TomcatMetrics) - Container startup configuration
 * issues
 *
 * <p>These tests would have caught the cgroup/SystemMetricsAutoConfiguration issues we encountered
 * during development.
 */
@Testcontainers
...
class DockerHealthTest {
  ...
  static void buildDockerImage() {
    ...
    try {
      ProcessBuilder pb =
          new ProcessBuilder("docker", "inspect", "guardianes-de-gaia-backend:latest");
      ...
      if (exitCode != 0) {
        ...
        pb =
            new ProcessBuilder(
                ...
                "build",
                "-t",
                "guardianes-de-gaia-backend:latest",
                "-f",
                "Dockerfile.dev",
                ".");
  ...
  void shouldStartBackendContainerSuccessfully() {
    ...
    assertFalse(logs.contains("ProcessorMetrics"), "Container should not fail on ProcessorMetrics");
    assertFalse(
  ...
  void shouldStartWithoutSpringContextFailures() {
    ...
    assertFalse(
        logs.contains("APPLICATION FAILED TO START"), "Application should start successfully");
    assertFalse(logs.contains("BUILD FAILURE"), "Maven build should not fail");
    assertFalse(
        logs.contains("Process terminated with exit code: 1"),
        "Process should not terminate with error");

    // And: Should contain successful startup indicators
    assertTrue(
        logs.contains("Started GuardianesApplication"), "Application should complete startup");
    assertTrue(logs.contains("Tomcat started on port"), "Tomcat should start successfully");
  }
  ...
  void shouldHandleJava17DockerCompatibility() {
    ...
    assertTrue(
        logs.contains("Java 17") || logs.contains("17.0"), "Container should be running Java 17");

    // And: Should not have Docker-specific Java compatibility issues
    assertFalse(
        logs.contains("UnsupportedOperationException"),
        "Should not have Java operation compatibility issues");
    assertFalse(
        logs.contains("IllegalAccessError"),
  ...
  void shouldHaveHealthyExternalDependencies() {
    ...
    // Note: We verify containers are running and accessible rather than using built-in health
    // checks
    ...
    assertFalse(
        logs.contains("Unable to connect"),
