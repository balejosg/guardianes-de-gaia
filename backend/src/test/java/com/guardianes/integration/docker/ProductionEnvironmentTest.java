 ...
 *
 * <p>This test validates the complete production-ready stack including: - Full Docker Compose
 * equivalent environment - All service interconnectivity - Monitoring stack integration - Load
 * balancer configuration - Production readiness validation
 */
@Testcontainers
...
class ProductionEnvironmentTest {
  ...
  @Container
  @SuppressWarnings("resource")
  static final GenericContainer<?> prometheus =
  ...
  void shouldConnectToAllExternalServices() throws Exception {
    ...
    long connectionRefusedCount =
        logs.lines().filter(line -> line.contains("Connection refused")).count();
    assertTrue(
        connectionRefusedCount < 10,
        "Should not have excessive connection refused errors (found: "
            + connectionRefusedCount
            + ")");

    // Check that the application actually started successfully
    assertTrue(
        logs.contains("Started GuardianesApplication"),
  ...
  void shouldHaveFullyFunctionalApiEndpoints() throws Exception {
    ...
    assertTrue(
        stepsResponse.body().contains("\"guardianId\":1"), "Should return correct guardian ID");
    assertTrue(
        energyResponse.body().contains("\"guardianId\":1"), "Should return correct guardian ID");
  }
  ...
  void shouldHaveAccessibleMonitoringEndpoints() throws Exception {
    ...
    assertTrue(
        healthResponse.body().contains("\"status\":\"UP\""),
        "Health monitoring should show UP status");
    assertTrue(
        metricsResponse.body().contains("\"names\":["), "Metrics should list available metrics");
    assertTrue(
        prometheusResponse.body().contains("# HELP"), "Prometheus should return proper format");
  }
  ...
  void shouldHandleAuthenticationCorrectly() throws Exception {
    ...
    HttpResponse<String> invalidResponse =
        httpClient.send(invalidRequest, HttpResponse.BodyHandlers.ofString());
    assertEquals(401, invalidResponse.statusCode(), "Invalid auth should fail");

    // No credentials should fail
    HttpRequest noAuthRequest =
        HttpRequest.newBuilder()
            .uri(URI.create(baseUrl + "/actuator/health"))
            .timeout(Duration.ofSeconds(10))
            .build();
    HttpResponse<String> noAuthResponse =
        httpClient.send(noAuthRequest, HttpResponse.BodyHandlers.ofString());
  ...
  void shouldHaveProperErrorHandling() throws Exception {
    ...
    String baseUrl = "http://localhost:" + backend.getMappedPort(8080);
    HttpRequest invalidDataRequest =
        HttpRequest.newBuilder()
            ...
            .timeout(Duration.ofSeconds(10))
            .build();
    HttpResponse<String> invalidDataResponse =
        httpClient.send(invalidDataRequest, HttpResponse.BodyHandlers.ofString());
    assertTrue(
        invalidDataResponse.statusCode() >= 400 && invalidDataResponse.statusCode() < 500,
