 ...
 *
 * <p>This test validates that our metrics configuration prevents the cgroup/ProcessorMetrics issues
 * that we encountered. It specifically tests: - SystemMetricsAutoConfiguration exclusion -
 * TomcatMetricsAutoConfiguration exclusion - Safe metrics are still available - No cgroup-related
 * startup failures
 */
@Testcontainers
...
class MetricsConfigurationTest {
  ...
  @Container
  @SuppressWarnings("resource")
  static final GenericContainer<?> backend =
  ...
  void shouldStartWithoutSystemMetricsFailures() {
    ...
    assertFalse(
        logs.contains("SystemMetricsAutoConfiguration"),
    ...
    assertFalse(
        logs.contains("Cannot invoke \"jdk.internal.platform.CgroupInfo.getMountPoint()\""),
        "Should not have cgroup access errors");
    assertFalse(
        logs.contains("because \"anyController\" is null"),
  ...
  void shouldStartWithoutTomcatMetricsFailures() {
    ...
    assertFalse(
        logs.contains("TomcatMetricsBinder.onApplicationEvent"),
    ...
    assertFalse(
        logs.contains("Error creating bean with name 'tomcatMetricsBinder'"),
  ...
  void shouldExcludeProblematicAutoConfigurations() {
    ...
    assertFalse(logs.contains("processorMetrics"), "ProcessorMetrics bean should not be created");

    // TomcatMetricsAutoConfiguration should be excluded
    assertFalse(logs.contains("TomcatMetricsBinder"), "TomcatMetricsBinder should not be created");

    // But DataSource and JPA should still be excluded (as per config)
    assertTrue(
        logs.contains("DataSourceAutoConfiguration")
  ...
  void shouldHaveActuatorFunctionalityDespiteExclusions() {
    ...
    assertTrue(
        logs.contains("Exposing") && logs.contains("actuator")
    ...
    assertFalse(logs.contains("No metrics available"), "Some metrics should still be available");

    // And: Health checks should work
    assertFalse(logs.contains("Health check failed"), "Health checks should function properly");
  }
  ...
  void shouldHaveSafeMetricsConfigurationForDocker() {
    ...
    assertFalse(
        logs.contains("Metrics configuration failed"), "Metrics configuration should not fail");

    // And: Should not have micrometer registry issues
    assertFalse(
        logs.contains("MeterRegistry"), "Should not have MeterRegistry configuration issues");
    ...
    assertFalse(
        logs.contains("Prometheus export failed"),
  ...
  void shouldShowCleanStartupWithoutMetricsErrors() {
    ...
    assertFalse(
        logs.contains("Error creating bean") && logs.contains("Metrics"),
