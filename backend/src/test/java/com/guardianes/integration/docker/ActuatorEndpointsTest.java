 ...
 *
 * <p>Validates that all Spring Boot Actuator endpoints are accessible and working correctly in the
 * Docker container environment. This test would have caught issues with missing actuator dependency
 * and metrics configuration problems.
 */
@Testcontainers
...
class ActuatorEndpointsTest {
  ...
  void shouldHaveAccessibleMetricsEndpoint() throws Exception {
    ...
    assertFalse(
        logs.contains("ProcessorMetrics"),
        "Should not have ProcessorMetrics initialization issues");
    assertFalse(logs.contains("CgroupInfo"), "Should not have cgroup access issues");
  }
  ...
  void shouldHaveAccessiblePrometheusEndpoint() throws Exception {
    ...
    assertFalse(
        body.contains("system_cpu_usage"),
  ...
  void shouldRequireAuthenticationForActuatorEndpoints() throws Exception {
    ...
    assertEquals(401, response.statusCode(), "Actuator endpoints should require authentication");
  }
  ...
  void shouldNotExposeProblematicSystemMetrics() throws Exception {
    ...
    assertFalse(body.contains("system.cpu.usage"), "Should not expose system CPU usage metrics");
    assertFalse(body.contains("process.cpu.usage"), "Should not expose process CPU usage metrics");

    // But: Should still include safe JVM metrics
    assertTrue(
        body.contains("jvm.memory.used") || body.contains("jvm."),
